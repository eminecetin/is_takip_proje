...
namespace is_takip_proje.Formlar
{
    public partial class FrmPersonelIstatistik : Form
    {
        ...
        private void FrmPersonelIstatistik_Load(object sender, EventArgs e)
        {
            ...
            LblSonGorev.Text = db.TblGorevler.OrderByDescending(x => x.ID).Select(x => x.Açiklama).FirstOrDefault();
            LblSonGorevDetay.Text = db.TblGorevler.OrderByDescending(x => x.ID).Select(x =>
